...
class BookRepositoryImpl @Inject constructor(
    ...
    override var favoriteBooks: List<BookDataModel> = listOf()
    ...
    override suspend fun buscarLivroPorNome(name: String): Result<List<BookDataModel>> = runCatching {
        bookApi.listarLivrosPorNome(name).results.map {
            bookMapper.mapResponseToDataModel(it, favoriteBooks)
        }
    }

    override suspend fun buscarLivroPorId(id: String?): Result<BookDetailsDataModel> = runCatching {
    ...
    override suspend fun buscarTodosLivros(page: Int): Result<List<BookDataModel>> = runCatching {
